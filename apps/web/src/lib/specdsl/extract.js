...
const OUT_DIR = path.resolve(__dirname, "../../..", "e2e/features");
...
for (const file of files) {
  ...
  const blocks = [...full.matchAll(regex)].flatMap((m) => (m.groups?.body ? [m.groups.body.trim()] : []));

  // 📝 For each Gherkin block found…
  blocks.forEach((content, i) => {
    // 🏷️ Generate output file name: same as source.md + index
    const out = path.join(OUT_DIR, `${path.basename(file, ".md")}.${i + 1}.feature`);
